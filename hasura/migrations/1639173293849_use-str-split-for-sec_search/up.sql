DROP FUNCTION IF EXISTS public.sec_search(text, date, date, text, text, text, text, text);
DROP FUNCTION IF EXISTS public.sec_search(text, date, date, text, text, text, text, text, text);
DROP FUNCTION IF EXISTS public.sec_search(text, date, date, text, text, text, text, text, text[]);
...
WHERE
  ...
  and (
    ...
    or a.company_cik = ANY(string_to_array(company_cik_eq,','))
  )
ORDER BY
  relevance desc
$BODY$;
